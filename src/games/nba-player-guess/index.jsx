...
const NBAPlayerGuess = ({ mode, description, instructions }) => {
  ...
  const { WelcomeModal } = useWelcomeModal("NBA Player Guess", instructions);
  ...
  useEffect(() => {
    ...
    const filtered = nbaPlayers.filter(player => 
      player.player.toLowerCase().includes(searchTerm.toLowerCase()) ||
      player.team.toLowerCase().includes(searchTerm.toLowerCase()) ||
      player.conference.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const selectRandomTarget = () => {
    ...
    setTargetPlayer(nbaPlayers[randomIndex]);
  };
  ...
  return (
    <GamePageLayout>
      <GameHeader title="NBA Player Guess" />
      ...
      <div style={{ textAlign: 'center', maxWidth: '1200px', margin: '0 auto', paddingTop: '5px' }}>
        <p style={{ margin: '0 0 8px 0', color: '#666', fontSize: '0.85rem' }}>
          2024 season data • Hints on 5th & 7th guess
        </p>
          ...
          {guesses.map((guess, index) => (
            ...
            >
              ...
              <div style={{
                padding: '8px',
                borderRadius: '3px',
                backgroundColor: getMatchColor(guess.conference, targetPlayer.conference),
                color: 'white',
                fontWeight: 'bold',
                textAlign: 'center'
              }}>
              ...
              <div style={{
                padding: '8px',
                borderRadius: '3px',
                backgroundColor: getMatchColor(guess.position, targetPlayer.position),
                color: 'white',
                fontWeight: 'bold',
                textAlign: 'center'
              }}>
              ...
              <div style={{
                padding: '8px',
                borderRadius: '3px',
                backgroundColor: getNumericMatchColor(guess.ppg, targetPlayer.ppg, 3),
                color: 'white',
                fontWeight: 'bold',
                textAlign: 'center'
              }}>
                {guess.ppg}{getArrow(guess.ppg, targetPlayer.ppg, 3)}
              </div>
              <div style={{
                padding: '8px',
                borderRadius: '3px',
                backgroundColor: getNumericMatchColor(guess.rpg, targetPlayer.rpg, 2),
                color: 'white',
                fontWeight: 'bold',
                textAlign: 'center'
              }}>
                {guess.rpg}{getArrow(guess.rpg, targetPlayer.rpg, 2)}
              </div>
              <div style={{
                padding: '8px',
                borderRadius: '3px',
                backgroundColor: getNumericMatchColor(guess.apg, targetPlayer.apg, 2),
                color: 'white',
                fontWeight: 'bold',
                textAlign: 'center'
              }}>
                {guess.apg}{getArrow(guess.apg, targetPlayer.apg, 2)}
              </div>
            </div>
          ))}
        </div>
